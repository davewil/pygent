...
def get_config_path() -> Path:
    ...
    """
    return Path.home() / ".config" / "pygent" / "config.toml"
...
Pygent is an AI-powered coding agent for the command line.
...
def get_setup_instructions(status: SetupStatus) -> str:
    ...
    if not status.has_api_key:
        ...
        lines.append("     pygent config set llm.api_key your-api-key")
        lines.append("")
    ...
    if not status.has_config_file:
        ...
        lines.append("   pygent config init")
        lines.append("")
    ...
    lines.append("   pygent chat")
    lines.append("")
    lines.append("For more help:")
    lines.append("   pygent help quickstart")
    lines.append("   pygent --help")
...
Pygent needs an API key to communicate with the AI model.
...
Run:
  ...
  pygent config set llm.api_key sk-ant-api03-...
...
def format_setup_complete_message(settings: Settings | None = None) -> str:
    ...
    lines = [
        ...
        "================================================================================",
        "",
        "You're all set to use Pygent.",
        "",
        "Quick commands:",
        "  pygent chat           Start an interactive session",
        "  pygent help           Show help topics",
        "  pygent tools          List available tools",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "  - Use 'pygent help <topic>' for detailed help",
            "",
            "================================================================================",
        ]
    )
...
def create_first_run_marker() -> None:
    ...
    """
    marker_path = Path.home() / ".local" / "share" / "pygent" / ".first_run_complete"
