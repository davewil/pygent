"""Logging infrastructure for chapgent using loguru.

This module provides file-based logging that doesn't interfere with the TUI.
Logs are written to ~/.local/share/chapgent/logs/ with rotation and compression.
"""
...
DEFAULT_LOG_DIR = Path("~/.local/share/chapgent/logs").expanduser()
DEFAULT_LOG_FILE = DEFAULT_LOG_DIR / "chapgent.log"
...
def setup_logging(
    ...
    Args:
        ...
        log_file: Path to log file. Defaults to ~/.local/share/chapgent/logs/chapgent.log
...
def redact_sensitive(content: str) -> str:
    ...
    content = re.sub(r"(CHAPGENT_API_KEY=)[^\s\n]+", r"\1[REDACTED]", content)
...
def get_log_files(days: int = 7) -> list[Path]:
    ...
    log_files: list[Path] = []
    for pattern in ["chapgent.log", "chapgent.log.*"]:
